 ...
 * 4/18/2018
 */
public class EmployeeMappingApp_180830 {
    ...
    @Test public void group_employees_by_department_then_list_employees_by_dept() {
        Map<Department, List<Employee>> collect = ee.stream().collect(groupingBy(Employee::getDepartment, toList()));
        collect.entrySet().forEach(ed ->{
            System.out.println(ed.getKey().getName());
            ed.getValue().forEach(e -> {
    ...
    @Test public void get_total_salary_by_dept() {
        Map<Department, Integer> collect = ee.stream().collect(groupingBy(Employee::getDepartment, summingInt(Employee::getSalary)));
        collect.entrySet().forEach(ed -> {
            System.out.println(ed.getKey().getName() + ":" + ed.getValue());
        });
    ...
    @Test public void get_employee_w_max_salary_by_dept() {
        ...
        collect.entrySet().stream()
                .forEach(e -> {
                    Optional<Employee> reduce = e.getValue().stream().reduce((f1, f2) -> f1.getSalary() > f2.getSalary() ? f1 : f2);
                    eeMaxSalByDept.put(e.getKey(), reduce.get());
                });
        assertEquals( "IT-C", eeMaxSalByDept.get( DepartmentIT ).getName() );
    }

    @Test public void get_employee_w_max_salary() {
        Optional<Employee> employee = ee.stream().reduce((e1, e2)-> e1.getSalary()>e2.getSalary()?e1:e2);
