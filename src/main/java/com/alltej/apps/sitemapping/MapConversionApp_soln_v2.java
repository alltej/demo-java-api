 ...
 * 4/9/2018
 */
public class MapConversionApp_soln_v2 {
    ...
    public static SiteValues convertResults(Collection<DbRecord> records) {
        ...
        collect.entrySet().stream().forEach(k -> {
            List<Meaurement> meaurements = new ArrayList<>();
            k.getValue().forEach(v -> meaurements.add(new Meaurement(v.getTime(), v.getValue())));
            map.put(k.getKey(), meaurements);
        });
        return null;
    }
    ...
    public static SiteValues convertResultsUsingCollectorsMap(Collection<DbRecord> records) {
        ...
        Map<String, List<Meaurement>> siteMeasurementsMap = collect.entrySet().stream().collect(toMap(
                e -> e.getKey(),
                e -> e.getValue().stream().map(v -> new Meaurement(v.getTime(), v.getValue())).collect(toList())
        ));

        return new SiteValues(siteMeasurementsMap);
    }
}
